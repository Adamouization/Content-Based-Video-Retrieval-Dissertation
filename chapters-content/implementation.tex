...
This inner pipeline of the entire system has to be repeated for every video in the database. For clarity and to illustrate the steps of this phase, the \textit{butterfly} video is used throughout this entire section to provide examples of the multiple offline colour-based feature extraction steps.
...
This dictionary is used at the end of the pipeline to determine which video was matched the most based on the number of occurrences in the dictionary, which can be plotted in a simple histogram using Matplotlib to visualise which video is matched the most. The final result is displayed in the form of a figure that includes the first frames of the query video and the matched video, along with the runtime in seconds and the accuracy $Accuracy = \frac{True \quad Positives}{Number \quad of \quad Matches}$ of the match (see Figure X for an example of the result figure).\\

todo add results figure here
...
\begin{figure}[h] 
\centerline{\includegraphics[width=\textwidth]{figures/implementation/UntitledDiagram.png}}
