Now that the requirements necessary to build the system have been formulated, potential solutions to fulfil the list of requirements from Chapter \ref{ch:chapter3} for each aspect of the system can now be analysed before choosing a final solution. The first section of this chapter will focus on different solutions for the system's pipeline, starting with the offline feature extraction phase, followed by the online retrieval phase, and ending with the database pre-processing phase. The second section will focus on general design decisions such as the programming language, the type of interface and the type of videos to use. These two sections are used to conclude with an outline of the final chosen solutions for each aspect of the system.
...
\begin{itemize}
    \item justify type of features to extract from DB videos
    \item justify what kind of histograms
    \item justify idea of creating a compact signature (araujo): average all histograms into 1
\end{itemize}
...
\begin{itemize}
    ...
    \item Manual Cropping: display the video have the user manually select the region of interest.
    \item Results:
    \begin{itemize}
